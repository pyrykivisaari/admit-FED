...
L = np.array([(d-Lled-Lpd)/4,Lpd,(d-Lled-Lpd)/4, \
    (d-Lled-Lpd)/4,Lled,(d-Lled-Lpd)/4])
L = L*1e-9
N = np.array([round((d-Lled-Lpd)/4*2),2*Lpd,round((d-Lled-Lpd)/4*2), \
    round((d-Lled-Lpd)/4*2),2*Lled,round((d-Lled-Lpd)/4*2)])
layers = ['algaas','gaas','algaas','algaas','gaas','algaas']
...
z0 = tf.set_source_coordinates(L,4)
...
for i, E in enumerate(Ep):
    pup_TE, pdown_TE, pup_TM, pdown_TM, P_TE, P_TM, rad_TE, rad_TM, \
        ...
        eps[i],mu[i],L,N,E,K[i],z0,dEF,T,eps_Ag[i])
    qte_wK[i] = rad_TE[:,int(round((Nc[3]+Nc[4])/2))]
    qtm_wK[i] = rad_TM[:,int(round((Nc[3]+Nc[4])/2))]
    P_TE_wK[i] = P_TE[:,int(round((Nc[2]+Nc[3])/2))]/E/q
    P_TM_wK[i] = P_TM[:,int(round((Nc[2]+Nc[3])/2))]/E/q
...
plt.pcolormesh(Epplot, Tplot, np.transpose(qte_wKint.real+qtm_wKint.real)*np.cos(Tplot/180*pi)*1e5, cmap='gnuplot', shading='gouraud')
...
plt.figure(figsize=(5.4,2.5))
plt.pcolormesh(Epplot, Tplot, np.transpose(-P_TE_wK.real-P_TM_wK.real)*np.cos(Tplot/180*pi)*1e5, cmap='gnuplot', shading='gouraud')
plt.xlabel('Photon energy (eV)')
plt.ylabel('Angle (deg.)')
plt.xlim(1.41,1.55)
...
spectrum_te_int = np.trapezoid(qte_w[:,Nc[3]:Nc[4]],\
        z[Nc[3]:Nc[4]],axis=1)
